...
OpenCode 是唯一同时支持 CLI、Web 和桌面应用的 AI 编程工具：

| 客户端 | 技术栈 | 适用场景 |
|--------|--------|----------|
| CLI | Bun + TypeScript | 终端工作流、自动化脚本 |
| Web | SolidJS + Vite | 远程访问、团队协作 |
| Desktop | Tauri + React | 本地开发、离线使用 |
...
# 使用 Haiku 快速迭代
opencode chat --model=claude-haiku
> 快速修复这个 typo
```
...
```bash
# 共享工作空间配置
opencode workspace export > workspace.json

# 团队成员导入
opencode workspace import workspace.json

# 使用相同的 AI 配置
opencode config sync --remote git@company.com:ai-config.git
```
...
```bash
# 创建项目工作空间
opencode workspace create --name "my-project" --path ./my-project

# 切换工作空间
opencode workspace switch my-project

# 查看会话历史
opencode session list --workspace my-project
...
export default {
  ai: {
    // 根据任务类型自动选择模型
    routing: {
      architecture: 'gpt-4',        // 架构设计
      coding: 'claude-3-sonnet',    // 代码编写
      review: 'claude-3-opus',      // 代码审查
      quick: 'claude-haiku',        // 快速任务
    },
  },
};
```
...
```typescript
// .opencode/templates/code-review.ts
...
```typescript
// .opencode/hooks/webhook.ts
export default {
  onSessionEnd: async (session: Session) => {
    // 发送到 Slack
    await fetch(process.env.SLACK_WEBHOOK!, {
      method: 'POST',
      body: JSON.stringify({
        text: `会话结束: ${session.title}`,
      }),
    });
  },
};
...
## 与 Claude Code / Codex CLI 对比分析

| 特性 | OpenCode | Claude Code | Codex CLI |
|------|----------|-------------|-----------|
| **核心优势** | 全栈平台 | 上下文理解 | 安全沙箱 |
| **底层模型** | 多模型支持 | Claude 4.5/4.6 | GPT-4o/o1/o3 |
| **开源状态** | 开源 | 闭源 | Apache 2.0 |
| **多客户端** | CLI+Web+Desktop | CLI | CLI |
| **桌面应用** | Tauri | 无 | 无 |
| **Web 界面** | 有 | 无 | 无 |
| **多模型** | 原生支持 | 单一模型 | 单一模型 |
| **会话持久化** | SQLite | 文件系统 | 文件系统 |
| **本地模型** | Ollama | 无 | 无 |
| **沙箱安全** | 系统沙箱 | 权限系统 | 原生多平台 |
...
| 场景 | 推荐工具 | 原因 |
|------|----------|------|
| 日常终端开发 | Claude Code | 上下文理解最强 |
| 安全敏感环境 | Codex CLI | 多平台沙箱隔离 |
| 需要桌面应用 | OpenCode | 唯一支持 |
| 团队协作 | OpenCode | Web 界面共享 |
| 本地模型 | OpenCode | Ollama 集成 |
| 多模型切换 | OpenCode | 原生支持 |
| 成本优化 | OpenCode | 可选便宜模型 |
